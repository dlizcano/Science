function [DOR, DORw, Sed, Frag, varargout] = Impact_Tier11(Hp, DrenageOrderThreshold, NameVar, varargin)
...
for j = 1:length(IDFun)
    ...
    LenFrag(j,1)    = nansum( eval(['Hp.Network.Variables.',NameVar{4},'( ID_FN )']) );
    LenFrag(j,2)    = nansum( LenDrenage_Umb( ID_FN ) );

    Order = eval(['Hp.Network.Variables.',NameVar{3},'( ID_FN )']);
    for k = 1:length(NumOrder)
        LenFrag(j,2 + k) = nansum( eval(['Hp.Network.Variables.',NameVar{4},'( ID_FN( Order == NumOrder(k) ) )']) );
    end
