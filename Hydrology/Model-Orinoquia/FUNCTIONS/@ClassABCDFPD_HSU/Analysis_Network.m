function [FuncNetwork, varargout] = Analysis_Network(Network)
% -------------------------------------------------------------------------
...
%% INPUT DATA 
if isempty(Network.PoNet), Network.PoNet = zeros(length(Network.ArcID),1); end
if isempty(Network.ArcBarrier), Network.ArcBarrier = zeros(length(Network.ArcID),1); end

%% VERIFICATION OF THE CONSISTENCY IN THE INPUT DATA
if ~isempty(Network.AccumLossVar) && isempty(Network.LossRate) && (Network.Mode==1),         error('inconsistency in variables with Losses'), end
if ~isempty(Network.AccumClipLossVar) && isempty(Network.LossRate) && (Network.Mode==1),     error('inconsistency in variables with Losses and Clipping'), end
if (isempty(Network.AccumClipLossVar) && isempty(Network.AccumLossVar)) && ~isempty(Network.LossRate) && (Network.Mode==1),     error('inconsistency in variables with Losses and Clipping'), end

%% FunctionalBranch
if Network.Mode
    % Position Network
    [FuncNetwork,O1,O2,O3,O4,O5,O6] = FunctionalBranch( Network.ArcID,...
                                        Network.FromNode, Network.ToNode, Network.ArcID_RM,...
                                            Network.ProVar, Network.ProVar, ... 
                                            Network.AccumVar, Network.AccumVar,...
                                            Network.AccumClipVar, Network.AccumClipVar,...
                                            Network.AccumLossVar, Network.AccumLossVar,...
                                            Network.AccumClipLossVar, Network.AccumClipLossVar,...
                                            Network.ArcBarrier, Network.CurrID, ...
                                            Network.LossRate, Network.ArcID_RM, Network.PoNet);
    ...
    PoNet_i     = zeros(length(Network.ArcID),1);
    [FuncNetwork, PoNet_i] = FunB_GetNetwork(Network.ArcID, Network.FromNode,...
                                    Network.ToNode, Network.ArcID_RM,...
                                    Network.ArcID_RM, Network.PoNet, PoNet_i);
