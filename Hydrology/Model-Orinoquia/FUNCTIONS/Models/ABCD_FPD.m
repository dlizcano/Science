function [VAc, Esc, ETR, StatesMT, StatesMF] =  ABCD_FPD(  Date, P, ETP, DemandSup, DemandSub, Returns,...
                                                ...
                                                IDExt, IDRet, ArcIDFlood, ParamExtSup, Sw, Sg, Vh, IDAq)
% -------------------------------------------------------------------------
...
%   IDRet
%       IDRetAgri       [Cat,1]         = ID of the HUA where to return Domestic Demand         [Ad]
...
%% Storages
% Storage of the Lateral flow between river and floodplain  
Ql          = zeros(length(Vh),1);

% Storage of the Return flow from floodplain to river
...
for i = 1:nTime
    ...
    [Esc(i,:), ETR(i,:), StatesMT(i,:,:)] = Thomas(P(i,:)' , ETP(i,:)' , a, b, c, d, Sw, Sg);
    
    % Rainfall [mm] to Volumen Basin [m^3] 
    Vsim        = (Esc(i,:)'.*Factor) .* (BasinArea);
    
    % Water avelable [mm] for the basin with  Floodplain
    ...
    % Total Underground Demand
    DemandSub_i = DemandSub(i,:,:) + bsxfun(@times, reshape(DemandSup(i,:,:), nBasin, nDemand) , 1 - ParamExtSup);
    ...
    for Dstrem = 1:length(ArcID_Downstream)
        ...
        % Mass Balance 
        [VAcc, Vh_i,Ql_i,Rl_i, PoPo, PoPoFlood] =    Network(ArcID,...              % ArcID Basin
