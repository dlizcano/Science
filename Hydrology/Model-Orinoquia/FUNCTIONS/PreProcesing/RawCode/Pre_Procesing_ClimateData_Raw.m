...
%     Id_Basin{i} = Posi(inpolygon(x, y, Basin(Poo(i)).X, Basin(Poo(i)).Y));
% end
% 
            ...
            for w = 1:length(Data(:,1))
                parfor k = 1:length(CodeBasin) 
                    if strcmp(UserData.ClimateVar{v},'Precipitation')
                        Values(w,k) = nanmean(PrecipitationFields(cell2mat(UserData.GaugesCatalog(posi,4)), cell2mat(UserData.GaugesCatalog(posi,5)), Data(w,:)', x(Id_Basin{k}), y(Id_Basin{k})));
                    elseif strcmp(UserData.ClimateVar{v},'Temperature')
                        Values(w,k) = nanmean(ETP_Thornthwaite(TemperatureFields(cell2mat(UserData.GaugesCatalog(posi,6)), Data(w,:)',z(Id_Basin{k}))));
                    elseif strcmp(UserData.ClimateVar{v},'Evapotranspiration')
                        Values(w,k) = nanmean(TemperatureFields(cell2mat(UserData.GaugesCatalog(posi,6)), Data(w,:)',z(Id_Basin{k})));
                    end
                end
